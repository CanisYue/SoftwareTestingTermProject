...
field_race_start_re = re.compile("^Data race on field ([^:]*):")
array_race_start_re = re.compile("^Data race on array element ([^:]*):")
race_op_re = re.compile("\s*([^ ]*) in thread ([^ ]*)")
location = "at ([^(]*)[(]([^:]*):([^)]*)[)]"
first_stack_re = re.compile("\s*> " + location)
next_stack_re = re.compile("\s*" + location)
lock_stack_re = re.compile("\s*- locked ([^ ]*) " + location)
thread_create_re = re.compile("\s*Thread ([^ ]*) created by thread([^\s]*)")
thread_main_re = re.compile("\s*Thread ([^ ]*) is the main thread");
race_end_re = re.compile("[}][}][}]")
...
for line in sys.stdin:
    ...
    if state == State.leg_initial:
        ...
        tmp = match.group(2).strip()
        regThread = re.compile(r'(\d+)')
        access.thread = re.findall(regThread, tmp)[0]
        regLock = re.compile(r'lock (.*?)')
        access.locks = re.findall(regLock, tmp)[0] if len(re.findall(regLock, tmp)) > 0 else ""
        # access.thread = match.group(2).strip()
...
print("Total Races Found: ", str(len(races)))
