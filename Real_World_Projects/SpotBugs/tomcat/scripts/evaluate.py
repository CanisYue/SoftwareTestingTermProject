        ...
        for cur_row in rows:
            if ":[line" in cur_row:
                ...
                if bug_type in conc_set:
                    ...
                    bug_location = project_name + "/" + cur_row[left_idx + 1 : right_idx + 1]
...
with open(output_file, mode = 'a') as filename:
    filename.write("\n\n----------Overall Results for Real World Projects----------\n")
    filename.write("Number of concurrency bugs found in Real World Projects: " + str(all_bugs_num) + "\n")
    filename.write("Number of concurrency bug types found in Real World Projects: " + str(len(all_res)) + "\n")
    filename.write("\n\n")
    for key in all_res:
